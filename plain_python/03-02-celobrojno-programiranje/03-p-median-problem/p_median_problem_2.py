...
# Load data
consumer_locations = pd.read_csv('plain_python/03-02-celobrojno-programiranje/03-p-median-problem/data/consumer_locations_01.csv', usecols= ['x','y'])
#print(consumer_locations)
facility_locations = pd.read_csv('plain_python/03-02-celobrojno-programiranje/03-p-median-problem/data/facility_locations_01.csv', usecols= ['x','y'])
#print(facility_locations)
building_costs = pd.read_csv('plain_python/03-02-celobrojno-programiranje/03-p-median-problem/data/building_costs_01.csv', usecols= ['building_costs'])
...
def distance(i:int, j:int)->float:
    ...
    return math.fabs(consumer.x-facility.x) + math.fabs(consumer.y-facility.y)
#print(distance(1,2))
...
for i in range(n):
    draw += geom_segment(mapping = aes(x=links.loc[i].x1, y=links.loc[i].y1, 
            xend=links.loc[i].x2, yend=links.loc[i].y1), color="cyan",  alpha = 0.3)
    draw += geom_segment(mapping = aes(x=links.loc[i].x2, y=links.loc[i].y1, 
